...
func _is_ore_valid(ore: Node2D) -> bool:
	return is_instance_valid(ore) and ore.is_inside_tree()

func _cleanup_invalid_ore() -> void:
	ore_queue = ore_queue.filter(_is_ore_valid)

func _release_current_ore() -> void:
	if current_ore in ore_queue:
		ore_queue.erase(current_ore)
	current_ore = null

func _deliver_ore() -> void:
	if _is_ore_valid(current_ore):
		current_ore.queue_free()  # or emit signal, add to inventory, etc.
	_release_current_ore()
...
func _on_range_body_exited(body: Node2D) -> void:
	if body in ore_queue and body != current_ore:
